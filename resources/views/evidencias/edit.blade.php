      ...
      <div class="card-body">
        <form method="POST" action="{{ route('evidencias.update', $evidencia) }}">
          ...
          <div class="mb-3">
            ...
            <input type="text" id="descricao" name="descricao" class="form-control @error('descricao') is-invalid @enderror" value="{{ old('descricao', $evidencia->descricao) }}" required>
            @error('descricao')
          ...
          <div class="mb-3">
            ...
            <select id="indicador_id" name="indicador_id" class="form-select @error('indicador_id') is-invalid @enderror" required>
              ...
              @foreach ($indicadores as $id => $descricao)
                <option value="{{ $id }}" @selected(old('indicador_id', $evidencia->indicador_id) == $id)>{{ $descricao }}</option>
              @endforeach
            </select>
            @error('indicador_id')
            <div class="invalid-feedback">{{ $message }}</div>
